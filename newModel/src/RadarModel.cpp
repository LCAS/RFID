...
RadarModel::RadarModel(const double nx, const double ny, const double resolution, const double sigma_power, const double sigma_phase, const double txtPower, const std::vector<double> freqs ) {
        _sizeX = nx;
        _sizeY = ny;
    ...
    void RadarModel::PrintPowProb(std::string fileURI, double rxPw, double f_i){
        ...
        PrintProb(fileURI, &prob_mat, f_i);
    }
    ...
    void RadarModel::PrintProb(std::string fileURI, Eigen::MatrixXf* prob_mat, double f_i){
        GridMap tempMap;      
        tempMap.setGeometry(Length(_sizeX,_sizeY), _resolution);
    ...
    void RadarModel::debugInfo(){
        ...
        std::cout << "We have "<< _map.getSize()(0) << " cells (x axis), " << _map.getSize()(1) << " cells (y axis)" <<std::endl;
        std::cout << "Area size is: "<< mlen(0) << " m. (x axis) , " << mlen(1) << " m. (y axis)" <<std::endl;
